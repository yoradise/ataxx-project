...
class Board {
    ...
    static int indexRow(int index) {
        return (int) (index / EXTENDED_SIDE);
    }

    /** Return the col of the INDEX. */
    static int indexCol(int index) {
        return (index % EXTENDED_SIDE);
    }
    ...
    void clear() {
        ...
        for (int i = 0; i < _board.length; i++) {
            if (i < EXTENDED_SIDE * 2
                    || i > EXTENDED_SIDE * EXTENDED_SIDE - 2 * EXTENDED_SIDE
                    || i % EXTENDED_SIDE < 2
    ...
    private void unrecordedSet(int sq, PieceColor v) {
        if (v == BLOCKED) {
            ...
            PieceColor thisPiece = get(sq);
            if (thisPiece == RED || thisPiece == BLUE) {
                incrPieces(thisPiece, -1);
            }
            if (v == RED || v == BLUE) {
                incrPieces(v, 1);
            }
    ...
    void makeMove(Move move) {
        ...
        for (int row = -1; row <= 1; row++) {
            for (int col = -1; col <= 1; col++) {
                int index = neighbor(move.toIndex(), col, row);
                if (get(index) == opponent) {
                    set(index, whoseMove());
                }
            }
        }
        ...
        if ((!canMove(RED) && !canMove(BLUE))
                || numPieces(RED) == 0 || numPieces(BLUE) == 0
    ...
    void undo() {
        ...
        while (piece != null || square != null) {
            unrecordedSet(square, piece);
            square = _undoSquares.pop();
    ...
    boolean legalBlock(char c, char r) {
        ...
        int loc = index(c, r);
        int col = loc % EXTENDED_SIDE;
        int row = (int) loc / EXTENDED_SIDE;
        int recol = EXTENDED_SIDE - col - 1;
        int rerow = EXTENDED_SIDE - row - 1;
        return (get(loc) == EMPTY
                && get(rerow * EXTENDED_SIDE + recol) == EMPTY
                && get(row * EXTENDED_SIDE + recol) == EMPTY
                && get(rerow * EXTENDED_SIDE + col) == EMPTY);
    }
    ...
    void setBlock(char c, char r) {
        ...
        int recol = EXTENDED_SIDE - col - 1;
        int rerow = EXTENDED_SIDE - row - 1;
        unrecordedSet(row * EXTENDED_SIDE + col, BLOCKED);
        if (recol != col) {
            unrecordedSet(row * EXTENDED_SIDE + recol, BLOCKED);
        }
        if (rerow != row) {
            unrecordedSet(rerow * EXTENDED_SIDE + col, BLOCKED);
        }
        if (recol != col && rerow != row) {
            unrecordedSet(rerow * EXTENDED_SIDE + recol, BLOCKED);
        }
        if (!canMove(RED) && !canMove(BLUE)) {
            _winner = EMPTY;
        }
